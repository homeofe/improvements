> Note (2026-08-22, claude-opus-5): Corrected four published claims that the tracked tree contradicts, and closed the installer gap behind one of them. (1) docs/AAHP-SWARM-v0.1.md and docs/AAHP-SWARM-v0.2.md still present "keep the swarm inside improvements or split it into a dedicated project" as an open decision (v0.1 section 9, open question 6, working position in section 16; v0.2 Decision Point in section 21). That decision is two months old: homeofe/aahp-swarm is a public repository carrying the scout, tester, risk and verdict roles both papers propose, plus a controller and a telemetry role, and the finding and swarm-run schemas they specify. Both papers now carry a Superseded status, a Superseded-by link and a banner; every line of the body text is unchanged, so the design record is preserved the same way the Grounded Reflection promotion preserved its v0.1 row. (2) docs/BUILDING-THE-FRAMEWORK.md still described the pre-v1.0 framework: four rules, five agents, five commands, four .llm files, and no .ai/GROUNDING.md anywhere in the architecture tree or the minimum file inventory. The 2026-07-14 promotion audit checked README.md and INTEGRATION.md and missed this file; the counts are now 5/6/6/5 and match `git ls-files`. (3) Its footer licensed the document under the MIT License. This repository ships the Apache License 2.0 (LICENSE, the README badge, the README License section, CONTRIBUTING.md, and the licence GitHub itself detects), and STATUS.md records the migration away from MIT. Corrected to Apache 2.0. The GitHub repository DESCRIPTION still ends with "MIT." and only the owner can change that; it is reported, not touched. (4) docs/hook-coverage.md called AAHP Verify "the required CI check" "which cannot be bypassed", and the aahp-verify.yml header called it a "non-bypassable backstop" that "stays a REQUIRED check". The branch-protection API for main returns no required_status_checks key at all, so a red run reports and does not block a merge. Both files now state what is true, keep the accurate part (the CI level has no AAHP_SKIP_VERIFY escape hatch), and name the configuration step that would close the gate. (5) scripts/init-framework.sh never copied .ai/GROUNDING.md, so a fresh install shipped five of the Grounded Reflection Layer's six files while the rule, the auditor agent and /challenge all pointed at a file that was not there. Added a guarded copy step. Proved by running the installer from origin/main into a clean target (.ai/ contains only handoff/) and from this branch (.ai/GROUNDING.md present). Sweep result, unchanged and verified in both directions: the tracked tree contains no private repository name, LAN address, host name or local path, and each of the nine deliberately kept MCP tool names is still present. Not fixed here and reported instead: running the repository's own scripts/aahp-manifest.sh regenerates MANIFEST.json with GROUNDING.md dropped from files and next_task_id demoted from the integer 2 back to the string "2" (aahp-manifest.sh line 212 always quotes it), so this handoff updated MANIFEST.json surgically instead. That script is the fleet-distribution deliverable and a push to main touching it triggers gate-sync, so the fix belongs in its own reviewed change.

> Note (2026-08-21, claude-opus-5): Removed the last hardcoded reference to a private MCP server from the framework docs, finishing the vendor-neutral pass that PR #21 started. Six files named one specific, non-public MCP server as the source of the routing and review tools (.claude/commands/review-cycle.md, .claude/commands/route.md, .claude/rules/multi-model.md, .llm/PROVIDERS.md, .llm/ROUTING.md, .ai/handoff/WORKFLOW.md); they now say "your configured MCP routing server" and describe the two review tools by capability instead of by that server's tool names. The conventional tool names in the mapping tables (gemini_run, codex_run, claude_run, local_llm_run, openclaw_run, perplexity_run, grok_run, chatgpt_run, local_llm_threadripper) are deliberately KEPT: they are documented publicly in elvatis/elvatis-mcp and elvatis/openclaw-cli-bridge-elvatis, so removing them would delete useful, already-published detail rather than close a disclosure. Verified in both directions: a repository-wide search for the removed server name returns zero hits, and each of the nine kept tool names is still present. No behaviour change - documentation and agent instructions only.
